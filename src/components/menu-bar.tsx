...
import { RadialMenu, RadialMenuItem, RadialMenuSubItem } from '@/components/ui/radial-menu';
...
export function MenuBar() {
  ...
  return <div className="absolute bottom-[max(24px,env(safe-area-inset-bottom))] left-1/2 z-10 -translate-x-1/2">
    <RadialMenu label="Add nodes" openLabel="Add a node" closeLabel="Close node menu">
      {categories.map(({ label, category, icon: Icon, items }) => (
        <RadialMenuItem key={category} label={label} icon={<Icon strokeWidth={1.7} />}>
          {items.map(node => {
            const NodeIcon = iconMapping[node.icon];
            return <RadialMenuSubItem key={node.id} aria-label={`Add ${node.title}`}
              icon={<NodeIcon aria-hidden="true" />}
              onSelect={() => addNode(createNodeByType({
                type: node.id,
