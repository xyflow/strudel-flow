  ...
  return (
    <>
      <svg
        ...
        className={`radial-menu__categories ${className}`}
      >
        <g data-active={active}>
          <g className="radial-menu__category-visual" aria-hidden="true">
            <path
              d={path}
              className="radial-menu__category-surface"
          ...
          <path
            ...
            aria-controls={active ? itemsId : undefined}
            data-radial-category
            className="radial-menu__category-target"
            onPointerEnter={(event) => {
              if (event.pointerType === 'mouse') menu.hover(id);
            }}
        ...
        >
          {items.map((item, itemIndex) => (
            <PositionContext.Provider
              key={isValidElement(item) ? (item.key ?? itemIndex) : itemIndex}
