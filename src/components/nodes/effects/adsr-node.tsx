  ...
  return (
    <WorkflowNode id={id} data={data}>
      <div className="flex justify-center gap-3 px-4 pt-1 pb-5">
        <ParameterKnob
          label="attack"
          value={Number(data.attack ?? 0.1)}
          min={0}
          max={2}
          step={0.01}
          onChange={(value) => update(id, { attack: String(value) })}
        />
        <ParameterKnob
          label="decay"
          value={Number(data.decay ?? 0.1)}
          min={0}
          max={2}
          step={0.01}
          onChange={(value) => update(id, { decay: String(value) })}
        />
        <ParameterKnob
          label="sustain"
          ...
          max={1}
          step={0.01}
          onChange={(value) => update(id, { sustain: String(value) })}
        />
        <ParameterKnob
          label="release"
          value={Number(data.release ?? 0.2)}
          min={0}
          max={2}
          step={0.01}
          onChange={(value) => update(id, { release: String(value) })}
        />
      </div>
    </WorkflowNode>
  );
