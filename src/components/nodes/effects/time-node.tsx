...
export function TimeNode(props: WorkflowNodeProps) {
  ...
  const dials = [
    { key: 'rate', label: 'fast', initial: 1, min: 0.125, max: 8, step: 0.125, unit: '×' },
    { key: 'lateOffset', label: 'late', initial: 0, min: 0, max: 1, step: 0.01 },
  ] as const;
  return <WorkflowNode id={props.id} data={props.data}><div className="space-y-4 px-4 pt-1 pb-5"><div className="flex gap-1 rounded-md bg-background/60 p-1">{(['forward', 'reverse', 'pingpong'] as const).map((direction, index) => <button key={direction}
    aria-pressed={(props.data.direction ?? 'forward') === direction} onClick={() => update(props.id, { direction })}
    className="nodrag flex-1 rounded-md px-3 py-2 text-[10px] text-muted-foreground transition hover:text-foreground aria-pressed:bg-muted aria-pressed:text-primary">{['Forward', 'rev', 'palindrome'][index]}</button>)}</div><div className="flex justify-center gap-4">{dials.map(dial => <ParameterKnob key={dial.key} label={dial.label}
