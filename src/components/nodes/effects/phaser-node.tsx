...
export function PhaserNode({ id, data }: WorkflowNodeProps) {
 ...
 return <WorkflowNode id={id} data={data}><div className="flex justify-center gap-3 px-4 pt-1 pb-5"><ParameterKnob label="phaser" value={Number(data.phaser ?? 1)} min={0.1} max={10} step={0.1} onChange={value => update(id, { phaserdepth: String(data.phaserdepth ?? 0.5), phaser: String(value) })} /><ParameterKnob label="phaserdepth" value={Number(data.phaserdepth ?? 0.5)} min={0} max={1} step={0.01} onChange={value => update(id, { phaser: String(data.phaser ?? 1), phaserdepth: String(value) })} /></div></WorkflowNode>;
}
