...
const waveforms = [
  ...
  { value: 'square', label: 'Square', path: 'M2 24 L2 8 L18 8 L18 24 L34 24 L34 8' },
];
    ...
    <WorkflowNode id={id} data={data}>
      <div className="nodrag flex min-w-52 flex-col gap-4 p-3">
        <div className="grid grid-cols-4 gap-1.5">
          {waveforms.map((wave) => (
            <button key={wave.value} aria-label={wave.label} title={wave.label} aria-pressed={sound === wave.value}
              onClick={() => handleValueChange(wave.value)}
              className={cn('grid h-12 place-items-center rounded-md border transition-colors focus-visible:outline-2 focus-visible:outline-ring', sound === wave.value ? 'border-primary/30 bg-primary/10 text-primary' : 'border-transparent bg-muted text-muted-foreground hover:text-foreground')}>
              <svg viewBox="0 0 36 32" className="h-7 w-8 fill-none stroke-current stroke-[1.5]" aria-hidden><path d={wave.path} /></svg>
            </button>
          ))}
        </div>
        <Select value={sound} onValueChange={handleValueChange}>
          <SelectTrigger aria-label="Voice sound" className="w-full border-0 bg-muted/50 text-xs">
            <SelectValue placeholder="Choose sound" />
          </SelectTrigger>
          <SelectContent>
            {waveforms.filter((wave) => !SYNTH_CATEGORIES.some((category) => category.options.includes(wave.value))).map((wave) => (
              <SelectItem key={wave.value} value={wave.value}>{wave.label}</SelectItem>
            ))}
